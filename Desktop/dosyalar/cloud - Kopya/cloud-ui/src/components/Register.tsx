...
import { UserPlus, User, Lock, Loader2, ArrowRight, ShieldCheck, Zap, Server, Globe, Cpu } from 'lucide-react';
import { api } from '../api';
    ...
    <div className="min-h-screen flex items-center justify-center p-6 bg-[#050506] relative overflow-hidden font-sans">
      ...
      <div className="absolute inset-0 z-0">
        ...
        <motion.div 
          ...
          className="absolute bottom-1/2 right-1/4 w-[500px] h-[500px] bg-brand/10 rounded-full blur-[140px]" 
        />
      </div>
      ...
      >
        <div className="glass-panel p-12 md:p-16 rounded-[4rem] border border-white/10 relative overflow-hidden bg-white/[0.01] backdrop-blur-[60px] shadow-[0_50px_100px_-20px_rgba(0,0,0,0.9)]">
          ...
          <div className="flex flex-col items-center mb-14">
            <motion.div 
              ...
              className="w-24 h-24 bg-white/[0.03] rounded-[2.5rem] flex items-center justify-center mb-8 shadow-inner border border-white/10 group cursor-pointer"
            >
              <div className="absolute inset-0 rounded-[2.5rem] bg-brand opacity-0 group-hover:opacity-10 transition-opacity" />
              <UserPlus className="w-12 h-12 text-brand" strokeWidth={2.5} />
            </motion.div>
            
            <h1 className="text-6xl font-black tracking-tighter mb-4 font-heading bg-clip-text text-transparent bg-gradient-to-b from-white to-white/60 uppercase">
              RE<span className="text-brand font-black italic">GISTER</span>
            </h1>
            ...
            <div className="flex items-center gap-3 bg-white/5 px-4 py-2 rounded-full border border-white/5 backdrop-blur-md">
              ...
              <span className="text-[10px] font-black tracking-[0.2em] text-gray-400 uppercase">Awaiting Protocol Initialization</span>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            <div className="space-y-3">
              <label className="text-[10px] font-black uppercase tracking-[0.3em] text-gray-500 px-1 ml-1 flex items-center gap-2">
                <User className="w-3 h-3 opacity-40 ml-[-2px]" />
                New Neural Identity
              </label>
              <div className="relative group">
                <Globe className="absolute left-6 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-600 group-focus-within:text-brand transition-all duration-300" />
                <input 
                  required 
                  type="text" 
            ...
            <div className="space-y-3">
              <label className="text-[10px] font-black uppercase tracking-[0.3em] text-gray-500 px-1 ml-1 flex items-center gap-2">
                <ShieldCheck className="w-3 h-3 opacity-40 ml-[-2px]" />
                Security Overpass
              </label>
              <div className="relative group">
            ...
            <button 
              disabled={loading} 
              className="w-full py-6 mt-4 bg-white text-black font-black uppercase text-sm rounded-3xl shadow-[0_25px_60px_-15px_rgba(255,255,255,0.2)] transition-all flex items-center justify-center gap-4 group active:scale-[0.98] disabled:opacity-50 tracking-[0.3em] relative overflow-hidden"
            >
              ...
              {loading ? (
                ...
                <>
                  <span className="translate-x-3 group-hover:translate-x-0 transition-transform duration-500">Initialize identity</span>
          ...
          <footer className="mt-16 pt-10 border-t border-white/5 flex flex-col items-center gap-8">
            <div className="flex items-center gap-3">
               <p className="text-gray-500 text-xs font-bold uppercase tracking-widest">Part of the grid?</p>
               <button onClick={onSwitch} className="text-brand font-black hover:text-white transition-colors text-xs uppercase tracking-widest underline underline-offset-8 decoration-2 decoration-brand/30 hover:decoration-white">Return to Portal</button>
            </div>
            
            <div className="grid grid-cols-3 gap-10 opacity-20 hover:opacity-100 transition-opacity duration-700">
               <div className="flex flex-col items-center gap-2">
                  <Zap className="w-5 h-5 text-white" />
                  <span className="text-[7px] font-black uppercase tracking-widest text-gray-400">Sync Stream</span>
               </div>
               <div className="flex flex-col items-center gap-2 border-x border-white/10 px-6">
                  <Server className="w-5 h-5 text-white" />
                  <span className="text-[7px] font-black uppercase tracking-widest text-gray-400">Provisioning</span>
               </div>
               <div className="flex flex-col items-center gap-2">
                  <Cpu className="w-5 h-5 text-white" />
                  <span className="text-[7px] font-black uppercase tracking-widest text-gray-400">Core Matrix</span>
               </div>
            </div>
          </footer>
        </div>
        ...
        <div className="mt-8 flex justify-center gap-8 opacity-20 text-[9px] font-black uppercase tracking-[0.4em] text-gray-500">
           <span>ENCRYPTED LAYER B3</span>
           <span>P2P MESH READY</span>
           <span>SECURE SHARDING</span>
        </div>
      </motion.div>
    </div>
  );
